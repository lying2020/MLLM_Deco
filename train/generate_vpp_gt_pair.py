#!/usr/bin/env python3
"""
生成 head 级别的真值对（ground truth pairs）- VPP版本
用于训练 1024 个 linear probe（32层 x 32个head）

VPP (Visual Prior Perturbation) 模型：
1. 只处理 POPE 基准且 label="yes" 的正样本
2. 对于每个正样本，通过添加 DDPM 噪声（30%或40%）生成负样本对
3. 计算每个head的概率增益（使用指数函数，避免log计算时的数值不稳定问题）
4. 计算语义先验偏置分数（公式C3）
5. 计算VPP输出g（公式D1，alpha=1.0, beta=0.0）
"""
...
def add_gaussian_noise_ddpm(image_tensor, timestep, num_timesteps=1000, device='cuda:0', verbose=False):
    ...
    参考 run_pope_eval_attention.py 中的实现
    ...
    if is_normalized:
        ...
        image_tensor_01 = (image_tensor - image_min) / image_range
        if verbose and timestep == 0:
    ...
    if num_timesteps != standard_timesteps:
        if num_timesteps == 1:
            ...
            actual_timestep = min(actual_timestep, standard_timesteps - 1)
    else:
...
def process_case_pope(
    ...
    extractor: HeadOutputExtractor, noisy_image_tensor: Optional[torch.Tensor] = None
) -> List[Dict]:
    """
    ...
    Args:
        ...
        num_heads: 每层的head数
        extractor: head输出提取器
        noisy_image_tensor: 可选的加噪图像tensor，如果提供则使用加噪图像，否则使用原始图像
    ...
    # 如果提供了加噪图像，使用加噪图像；否则使用原始图像
    if noisy_image_tensor is not None:
        image_tensor = noisy_image_tensor
...
def main():
    ...
    # VPP版本：只处理POPE基准且label="yes"的正样本
    print(f"  原始case数: {len(all_cases)}")
    cases = []
    for case in all_cases:
        # 判断是否为POPE类型（非CHAIR类型）
        case_type = "CHAIR" if "describe" in case.get("text", "").lower() else "POPE"
        if case_type == "POPE":
            # 获取label
    ...
    print(f"✓ 筛选后（POPE且label='yes'）: {len(cases)} 个cases")

    if len(cases) == 0:
        print("⚠️  警告: 没有找到符合条件的case（POPE且label='yes'）")
        return
    ...
    # 处理每个case
    print(f"\n[4/5] 处理cases并生成真值对（VPP版本：每个正样本生成正负样本对）...")
    ...
    empty_case_stats = {
        ...
        'cases_with_pairs': 0,
        'total_pairs': 0,
        'positive_pairs': 0,  # 正样本生成的真值对
        'negative_pairs': 0   # 负样本（加噪）生成的真值对
    }
    ...
    for idx, case in enumerate(tqdm(cases, desc="处理进度")):
        ...
        print(f"  图像路径: {image_path}")
        print(f"  文本 (text): {case_text}")
        print(f"  原始标签 (label): {case_label}")
        print(f"{'='*80}")

        try:
            empty_case_stats['processed_cases'] += 1

            # VPP版本：为每个正样本生成两个case（原始图像和加噪图像）
            # 1. 处理原始图像（正样本，label="yes"）
            print(f"  [1/2] 处理原始图像（正样本，label='yes'）...")
            positive_case = case.copy()  # 保持原始case不变
            positive_pairs = process_case_pope(
                model, tokenizer, image_processor, positive_case, args.coco_root,
                device, conv_mode, num_layers, num_heads, extractor, noisy_image_tensor=None
            )

            if len(positive_pairs) > 0:
                empty_case_stats['positive_pairs'] += len(positive_pairs)
                print(f"    ✓ 正样本生成了 {len(positive_pairs)} 个真值对")
            else:
                print(f"    ⚠️  正样本未生成任何真值对")

            # 2. 处理加噪图像（负样本，label="no"）
            print(f"  [2/2] 处理加噪图像（负样本，label='no'）...")
            # 加载原始图像并添加DDPM噪声
            image = load_image(image_path)
            image_tensor = image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]

            # 添加DDPM噪声（timestep=300对应30%噪声，或400对应40%噪声）
            noisy_image_tensor = add_gaussian_noise_ddpm(
                image_tensor,
                timestep=DDPM_NOISE_TIMESTEP,
                num_timesteps=1000,
                device=device,
                verbose=(idx == 0)  # 只在第一个case时输出详细信息
            )

            # 创建负样本case（label改为"no"）
            negative_case = case.copy()
            negative_case["label"] = ["no"] if isinstance(case.get("label"), list) else "no"

            negative_pairs = process_case_pope(
                model, tokenizer, image_processor, negative_case, args.coco_root,
                device, conv_mode, num_layers, num_heads, extractor, noisy_image_tensor=noisy_image_tensor
            )

            if len(negative_pairs) > 0:
                empty_case_stats['negative_pairs'] += len(negative_pairs)
                print(f"    ✓ 负样本生成了 {len(negative_pairs)} 个真值对")
            else:
                print(f"    ⚠️  负样本未生成任何真值对")
            ...
            if len(all_pairs) == 0:
                ...
                print(f"  ⚠️  警告: Case #{case_id} 处理完成，但未生成任何真值对")
            else:
                empty_case_stats['cases_with_pairs'] += 1
                empty_case_stats['total_pairs'] += len(all_pairs)
                print(f"  ✓ Case #{case_id} 总共生成了 {len(all_pairs)} 个真值对（正样本: {len(positive_pairs)}, 负样本: {len(negative_pairs)}）")

            all_ground_truth_pairs.extend(all_pairs)
    ...
    print(f"\n{'='*80}")
    print(f"处理统计:")
    print(f"  总case数（正样本）: {empty_case_stats['total_cases']}")
    ...
    print(f"  正样本真值对数量: {empty_case_stats['positive_pairs']}")
    print(f"  负样本真值对数量: {empty_case_stats['negative_pairs']}")
    print(f"{'='*80}")
    ...
    # 按case类型统计（VPP版本只有POPE类型）
    pope_count = sum(1 for p in all_ground_truth_pairs if p["case_type"] == "POPE")
    print(f"\n按case类型统计:")
    print(f"  POPE类型真值对: {pope_count}")

    # 统计正负样本数量（通过case_id和label判断）
    # 注意：这里我们无法直接从真值对中区分正负样本，因为process_case_pope已经处理了label
    # 但我们可以通过统计信息来了解
    print(f"\nVPP正负样本统计:")
    print(f"  正样本真值对: {empty_case_stats['positive_pairs']}")
    print(f"  负样本真值对: {empty_case_stats['negative_pairs']}")
